describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let buttonTextHandle = await buttonHandle.getProperty('innerText');
    let buttonText = await buttonTextHandle.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    const allProductItem = await page.$$('product-item');

    for (let i = 1; i < allProductItem.length; i++) { // the first one has been selected, so we start from the second element
      const product = allProductItem[i];
    ...
    const countHandle = await count.getProperty('innerText');
    const countVal = await countHandle.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < allProducts.length; i++) {
      ...
      // Expect it to still say "Remove from Cart"
      expect(buttonText).toBe('Remove from Cart');

    }

    const countElement = await page.$('#cart-count');
    const countTextHandle = await countElement.getProperty('innerText');
    const countVal = await countTextHandle.jsonValue();
    expect(countVal).toBe('20');
  }, 20000);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const countElement = await page.$('#cart-count');
    const countHandle = await countElement.getProperty('innerText');
    const countVal = await countHandle.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const count = await page.$('#cart-count');
    const countHandle = await count.getProperty('innerText');
    const countText = await countHandle.jsonValue();
